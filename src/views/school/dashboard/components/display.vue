<template>
  <div>
    <!-- 使用动态 id 值 -->
    <div :id="'mychart-' + groupId" :style="{ float: 'left', width: '100%', height: '400px' }" class="echart"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getGroupDataInfo() {
      getGroupData(this.groupId).then(res => {
    ...
    initEcharts() {
      // 使用动态 id 值
      const chartContainerId = 'mychart-' + this.groupId
